"""
term_audit/study_scope_audit.py

STUDY_SCOPE_AUDIT — a reasoning framework for AI to treat scientific
claims as measurement artifacts bounded by instrument, protocol, and
regime constraints — not as timeless laws.

Purpose: training-data correction for AI models that treat recent
studies as prima facie law rather than as scope-bounded measurements.

This module does NOT invalidate peer review, the scientific method,
or empirical evidence. It restricts HOW those results travel outside
the scope where they were measured. A study is TRUE WITHIN its scope,
SILENT outside, and becomes FALSE only when applied outside its scope
as if it held there.

Shipped in AUDIT_15 on top of AUDIT_14 Part B. CC0. Stdlib-only.
Self-contained — no import dependencies on the rest of the framework.
"""
...
class ProtocolAudit:
    ...
    excluded_conditions: List[str]         # what was FILTERED OUT
    ...
    def protocol_filters(self) -> List[str]:
...
class DomainCouplingAudit:
    ...
    def coupling_summary(self) -> Dict[str, str]:
...
class RegimeAudit:
    ...
    regime_state: Regime
    regime_drift_indicators: List[str]     # observed shifts in the baseline
...
class CausalModelAudit:
    ...
    confounders_unmeasured: List[str]      # the known-unknowns
    unknown_unknowns_acknowledged: bool    # did the study admit blindness?
    alternative_frames_considered: List[str]
...
class ScopeBoundary:
    ...
    in_scope_conditions: List[str]         # explicit deployment conditions
    edge_conditions: List[str]             # known boundary regions
    out_of_scope_conditions: List[str]     # explicit exclusions
    undeclared_scope: List[str]            # conditions the study never addressed
    extrapolation_claims: List[str]        # where the study REACHED beyond data
    ...
    def scope_status_for(self, deployment_context: Dict) -> ScopeStatus:
        ...
        The matcher below is deliberately naive — domain-specific
        deployment contexts need domain-specific matching. Callers
        override `_context_matches` with their own logic for real use.
        """
        # Order matters: OUT_OF_SCOPE wins over EDGE which wins over
        # IN_SCOPE — a deployment that matches both "in" and "out"
        # conditions is out. Treating "out-of-scope" as dominant is
    ...
    def _context_matches(self, ctx: Dict, conditions: List[str]) -> bool:
        """Placeholder naive matcher: any deployment-context key that
        appears (as a substring) in any condition string counts as a
        match. Intended to be overridden by domain-specific subclasses
        with real semantic matching (temperature ranges, population
        definitions, substrate types, etc.).

        Underscores and hyphens in the key are normalized to spaces
        so that `dirty_urban` in a context key matches `dirty urban`
        in a condition string. This is the only normalization done;
        anything more is the caller's responsibility.
        """
        if not conditions:
            return False
...
HISTORICAL_CASES: Dict[str, Dict] = {
    "geocentrism": {
...
def _demo() -> None:
    """A worked example applying every layer to a toy study claim."""
    audit = StudyScopeAudit(
        claim="Polymer X lasts 50 years outdoors",
        citation="Lab Study Journal 2018 vol 42",
        instrument=InstrumentAudit(
            instrument_name="UV accelerated weathering chamber",
            physical_quantity_measured="tensile strength retention under UV + temp cycling",
            measurement_range=(0.0, 1.0),
            resolution=0.01,
            noise_floor=0.005,
            sampling_rate_hz=None,
            spatial_resolution="coupon-level",
            calibration_source="tensile reference block",
            calibration_traceability="NIST tensile standards",
            drift_rate="manufacturer-spec 0.5% / year",
        ),
        protocol=ProtocolAudit(
            sample_preparation="clean lab coupon, defect-free",
            environmental_controls={"temp": 22, "humidity": 0.5},
            excluded_conditions=["ozone exposure", "particulate abrasion", "biological fouling"],
            control_group_definition="untreated identical coupons",
            measurement_duration="1000 hr accelerated weathering",
            replication_count=3,
            blinding=False,
            pre_registration=False,
        ),
        coupling=DomainCouplingAudit(
            physical_domain="polymer weathering",
        ...
        regime=RegimeAudit(
            assumed_baseline="typical outdoor exposure matches 1000 hr chamber",
            baseline_validity_window="chamber conditions only",
            regime_state=Regime.DRIFTING,
            regime_drift_indicators=["rising UV index", "warming climate baseline"],
            extrapolation_horizon="50 years",
        ),
        causal_model=CausalModelAudit(
            causal_frame="deterministic degradation under UV + temp",
            confounders_identified=["UV dose", "temperature"],
            confounders_controlled=["UV dose", "temperature"],
            confounders_unmeasured=["ozone", "particulate", "microbial", "ionizing UV-C-like"],
            unknown_unknowns_acknowledged=False,
            alternative_frames_considered=[],
        ),
        scope=ScopeBoundary(
            in_scope_conditions=["lab bench polymer coupon"],
            edge_conditions=["mild outdoor exposure"],
            out_of_scope_conditions=["dirty urban", "coastal salt spray", "high-ozone industrial"],
            undeclared_scope=["actual 50-year outdoor service"],
            extrapolation_claims=["50 years outdoors"],
        ),
        deployment_context={"dirty_urban": True, "50_year_outdoor_service": True},
    )

    report = audit.audit_report()
    print("=" * 72)
    print(f"STUDY SCOPE AUDIT: {report['claim']}")
    print("=" * 72)
    print(f"  citation:               {report['citation']}")
    print(f"  coupling:               {report['coupling_profile']}")
    print(f"  regime risk:            {report['regime_risk']}")
    print(f"  causal fragility:       {report['causal_frame_fragility']}")
    print(f"  scope status:           {report['scope_status_for_deployment']}")
    print(f"  verdict:                {report['verdict']}")
    print()
    print("instrument blind spots:")
    for b in report["instrument_blind_spots"]:
        print(f"  - {b}")
    print()
    print("protocol filters:")
    for f in report["protocol_filters"][:4]:
        print(f"  - {f}")
    print(f"  ... and {len(report['protocol_filters']) - 4} more")
