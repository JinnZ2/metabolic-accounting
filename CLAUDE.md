...
| Translate a money-as-signal coupling context into a signal-quality discount on money-denominated accounting | `money_signal/accounting_bridge.py` (`signal_quality`, `coupling_assumption_flags`, `adjust_glucose_flow`, `context_from_verdict_signal`) | `tests/test_money_signal_accounting_bridge.py`, `docs/AUDIT_12.md`, `docs/AUDIT_13.md` |
| Model investment as substrate-of-self committed against substrate-of-future (7 substrates, conversion/realization matrices, time binding, derivative distance, money dependency) | `investment_signal/README.md`, `investment_signal/coupling.py::assemble_investment_signal` | `investment_signal/dimensions.py`, `tests/test_investment_signal.py`, `docs/AUDIT_13.md` |
| See the forward-priority list for this repo | `Todo.md` (three priorities + audit cross-reference) | `docs/AUDIT_12.md`, `docs/AUDIT_13.md`, `docs/AUDIT_14.md` |
| Audit a scientific study as a scope-bounded measurement (instrument, protocol, regime, causal frame, deployment match) | `term_audit/study_scope_audit.py` (6 layers + 5 historical anchors) | `tests/test_study_scope_audit.py`, `docs/AUDIT_15.md` |
| Reason about the cost of pretending a claim holds beyond its measured scope (paired with study_scope_audit) | `term_audit/informational_cost_audit.py` (geocentric worked example + `CostLedger` + `compare()`) | `tests/test_informational_cost_audit.py`, `docs/AUDIT_16.md` |
| Stub for cross-observer signal-asymmetry reporting (production work lives in the sister repo) | `distributional/signal_asymmetry.py` (`ObserverAsymmetryReport`, `observer_delta`, 4 literature anchors) | `tests/test_signal_asymmetry.py`, `docs/AUDIT_14.md` Part C |
